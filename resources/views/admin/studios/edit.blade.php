                        ...
                        <div class="row gy-4 ">
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="vendor_id" id="vendor_id" class="form-select">
                                    ...
                                    @foreach ($vendors as $v)
                                        <option value="{{ $v->id }}" @selected($v->id == $studio->vendor_id)>{{ $v->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-3">
                            ...
                            <div class="col-md-6">
                                <div class="w-100 d-flex justify-content-between mb-2">
                                    ...
                                    <button data-bs-toggle="modal" onclick="addNewServices('{{$studio->id}}')" data-bs-target="#staticBackdrop" type="button" class="btn btn-sm btn-warning">Add New</button>
                                </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="input-group">
                                                     <input type="text" class="form-control" name="charge{{$s->id}}" id="charge{{$s->id}}" value="{{$s->charge}}" />
                                                     <button type="button" onclick="updateCharge({{$s->id}})" class="btn btn-gradient">Update</button>
                                                </div>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                ...
                                                <a href="{{route('delete_s_service', $s->id)}}" class="btn btn-sm btn-danger ">Delete</a>
                                            </td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                            ...
                            <div class="col-md-2">
                                ...
                                <select class="form-select" name="opens_at">
                                    <option value="">---Select---</option>
                                    @foreach($stimes as $s)
                                        <option value="{{$s}}" @selected( date('H:i', strtotime($studio->opens_at))  == $s)>{{$s}}</option>
                                    @endforeach
                            ...
                            <div class="col-md-2">
                                <label for="">Booking End At</label>
                                 <select class="form-select" name="ends_at">
                                    <option value="">---Select---</option>
                                    @foreach($etimes as $s)
                                        <option value="{{$s}}" @selected( date('H:i', strtotime($studio->ends_at))  == $s)>{{$s}}</option>
                                    @endforeach
                            ...
                            <div class="col-md-2">
                                ...
                                <select name="country_id" onchange="getStates(event)" id="country_id" class="form-select">
                                    ...
                                    @foreach ($countries as $c)
                                        <option value="{{ $c->id }}" @selected($studio->country_id == $c->id)>{{ $c->country }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-2">
                                ...
                                <select name="state_id" onchange="getCity(event)" id="state_id" class="form-select">
                                    <option value="" selected @disabled(true)>---Select---</option>
                                     @foreach ($states as $s)
                                        <option value="{{ $s->id }}" @selected($studio->state_id == $s->id)>{{ $s->state }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-2">
                                ...
                                <select name="district_id" id="city_id" class="form-select">
                                    <option value="" selected @disabled(true)>---Select---</option>
                                     @foreach ($cities as $s)
                                        <option value="{{ $s->id }}" @selected($studio->district_id == $s->id)>{{ $s->city }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-2">
...
<script>
    const addNewServices = (id) => {
        $.post("{{route('get_rest_services')}}", {studio_id : id}, function(res){
    ...
    <script>
        ...
        const updateCharge = (id) => {
            ...
            $.post("{{route('studio.update_s_service')}}", {id : id, charge : charge}, function(res){
                location.reload();
            })
        }
    </script>
